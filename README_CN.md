## 🌏 Seeed Studio 机器人学院

欢迎来到 **Seeed Studio 机器人学院** - 这是您通往机器人教育的综合门户。在这里，您将找到关于 ROS、机器人仿真、硬件控制和高级机器人应用的结构化学习路径。

### 📦 硬件 Wiki

提供机器人硬件的详细指南、数据手册和设置教程。

[Seeedstudio Wiki](https://wiki.seeedstudio.com/robotics_page/)

### 🎓 机器人学院 (Robotics Academy)

涵盖 ROS、仿真、强化学习和高级机器人控制的综合课程体系。

#### 核心软件与仿真
| 课程 | 难度 | 时长 | 简介 |
| :--- | :--- | :--- | :--- |
| [**ROS基础入门八讲**](./Seeed_ROS1_Turorial/1.中文/1.ROS基础入门八讲/README.MD) | 🟢 初级 | 8h | 学习节点、话题、服务，并 DIY 移动机器人。 |
| [**ROS机器人仿真七讲**](./Seeed_ROS1_Turorial/1.中文/2.ROS机器人仿真七讲/README.MD) | 🟢 初级 | 8h | 从建模到自主导航。 |
| **ROS 2 Humble** | 🟡 中级 | 8h | 使用 Python 掌握节点、话题、服务和动作 (Action)。 |
| **MoveIt 1/2** | 🟢 初级 | 6h | 导入自定义机械臂并实现正逆运动学。 |
| **Pinocchio** | 🟡 中级 | 6h | 学习 Pinocchio 框架，开发机械臂动力学任务。 |
| **NVIDIA Isaac Sim**| 🔴 高级 | 12h | Sim2Real 工作流、USD 管道和环境创建。 |
| **MuJoCo** | 🔴 高级 | 10h | 敏捷机器人控制的动力学建模与仿真。 |
| **强化学习 (RL)** | 🟣 困难 | 20h | PPO、DRL 训练管道以及 Jetson 部署。 |

#### 机器人应用
| 课程 | 难度 | 简介 |
| :--- | :--- |:--- |
|[**ReachyMini 桌面机器人13讲:**](./Seeed_ReachyMini_Tutorial/1.中文/README.md)| 🟡 中级 | ReachyMini 课程 - 基础操作、接口教程及二次开发。|
|**人形机器人:**|🟢 初级 | 小型舵机人形机器人课程（硬件驱动、动作与舞蹈编排）。|
|**机械臂:**|🟡 中级 | StarAI 机械臂系统 - 运动学、动力学、模仿学习及基于 VLA 的强化学习。|
|**轮足机器人:**|🟡 中级 | Stackforce 课程 - 混合机器人的平衡控制与 LQR 运动规划。|


---

<p align="center">
  Generated from Seeed Studio Documentation
</p>
